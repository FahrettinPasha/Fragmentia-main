        ...
        if GAME_STATE == 'LOADING':
            ...
            if loading_stage >= len(fake_log_messages):
                ...
                if loading_progress >= 1.0:
                    init_game()
                    # ── GUTTER RUNNER: Bölüm 1 ve 2'yi özel döngüde çalıştır ──
                    _gr_type = EASY_MODE_LEVELS.get(current_level_idx, {}).get('type', '')
                    if _gr_type == 'gutter_runner':
                        import gutter_runner as _gr
                        _gr_result = _gr.run(current_level_idx, screen, clock, save_manager)
                        if _gr_result == 'completed':
                            save_manager.unlock_next_level('easy_mode', current_level_idx)
                            current_level_idx += 1
                            start_loading_sequence('PLAYING')
                        elif _gr_result == 'died':
                            start_loading_sequence('PLAYING')
                        else:  # 'quit'
                            running = False
                    else:
